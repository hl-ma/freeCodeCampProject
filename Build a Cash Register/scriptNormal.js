...
const checkCashRegister = () => {
    const priceToCents = Math.round(price * 100);
    const cashToCents = Math.round(Number(cashInput.value) * 100);
    const cidToCents = Math.round(cid.reduce((acc, curr) => acc + curr[1], 0) * 100);
    ...
    if (changeDue > cidToCents) return { expression: 'RegisterInsufficientFunds', status: 'INSUFFICIENT_FUNDS', change: [] };
    ...
    const changeToGive = [];
    currencyUnits.map(([currency, unitValue]) => {
        let currencyInDrawerToCents = Math.round(
            cid.find(el => el[0] === currency)[1] * 100
        );
        let currencyUsed = 0;
    ...
    let cidRemaining = cidToCents;
    changeToGive.map(([currency, value]) => {
        for (let cidCurrency of cid) {
            if (cidCurrency[0] === currency) {
                cidCurrency[1] -= value;
...
const updateRegisterDisplay = () => {
    ...
    const drawerInfo = cid.map(
        (el) => currencyNameMap[el[0]] + ': ' + el[1].toFixed(2)
    ).join('<br>');
...
purchaseBtn.addEventListener('click', () => {
    const { expression, status, change } = checkCashRegister();
